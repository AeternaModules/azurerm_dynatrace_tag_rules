...
output "dynatrace_tag_rules_log_rule" {
  ...
  value       = { for k, v in azurerm_dynatrace_tag_rules.dynatrace_tag_rules : k => one(v.log_rule) if v.log_rule != null && length(v.log_rule) > 0 }
}
output "dynatrace_tag_rules_metric_rule" {
  ...
  value       = { for k, v in azurerm_dynatrace_tag_rules.dynatrace_tag_rules : k => one(v.metric_rule) if v.metric_rule != null && length(v.metric_rule) > 0 }
}
